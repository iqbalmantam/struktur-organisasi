...
def build_org_graph(df: pd.DataFrame) -> graphviz.Digraph:
    ...
    graph.attr(
        rankdir="TB", bgcolor="transparent", nodesep="0.2", ranksep="0.35",
        size="8,8", ratio="compress",
    )
    graph.attr(
        "node", shape="box", style="rounded,filled", fontname="Helvetica",
        fontcolor="white", color="#1e293b", margin="0.08,0.05", height="0.3",
    )
    ...
    for _, row in df.iterrows():
        ...
        graph.node(name, label, fillcolor=color_for(row["Level"]), fontsize="8")
