...
if "Atasan 2" in data_df.columns:
    data_df["Atasan 2"] = data_df["Atasan 2"].fillna("").astype(str).str.strip()
    data_df.loc[data_df["Atasan 2"].isin(["-", "—", "–"]), "Atasan 2"] = ""
...
def build_org_graph(
    ...
    # garis kedua (informal/dotted-line) ke Atasan 2, kalau kolomnya ada dan terisi
    if "Atasan 2" in df.columns:
        for row in expanded_rows:
            child, parent2 = row["Nama"], row.get("Atasan 2", "")
            if parent2 and parent2 in names_seen and parent2 != row["Atasan"]:
                graph.edge(
                    parent2, child, style="dashed", color="#f59e0b",
                    constraint="false",
                )
...
with tab_chart:
    ...
    try:
        ...
        <div style="position: relative; width: 100%;">
            ...
            ">
                <div id="orgSvgWrap" style="zoom: 1; display: inline-block;">{svg_source}</div>
            </div>
