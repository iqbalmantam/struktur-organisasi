...
def build_org_graph(df: pd.DataFrame, focus_level: str | None, rankdir: str) -> tuple[graphviz.Digraph, set]:
    ...
    node_count = df_use["Nama"].nunique()
    fsize = "10" if node_count <= 40 else "9" if node_count <= 80 else "8"

    graph = graphviz.Digraph()
    graph.attr(
        rankdir=rankdir, bgcolor="transparent", nodesep="0.25", ranksep="0.5",
        splines="curved",
    )
    graph.attr(
...
with tab_chart:
    ...
    org_graph, matched = build_org_graph(
        data_df, st.session_state.focus_level, rankdir
    )
